...
* `簡易カラオケ字幕.obj2` をお使いの AviUtl2 の `Script` フォルダに入れてください。
	* 通常 `C:\ProgramData\aviutl2\Script` です。
...
* 表示したいテキストをオブジェクト設定パネルの [字幕] 欄に入力してください。  
  ...
  例えば以下のように入力すると、**ここに<ruby>歌<rt>か</rt></ruby><ruby>詞<rt>し</rt></ruby>を<ruby>入<rt>にゅう</rt></ruby><ruby>力<rt>りょく</rt></ruby>します** と表示されます。
	...
	* [ルビ] 欄: `[か][し][にゅう][りょく]`
* ルビを記号 `/` で区切ることで、メインテキストの文字毎にルビを割り付けることができます。  
  例えば [字幕] 欄に `[堕天使]`、[ルビ] 欄に `[だ/てん/し]` と入力すると、それぞれ `[堕][天][使]` `[だ][てん][し]` と入力した場合と同じ結果が得られます。
...
* 2025年11月10日
	* 特殊記号 `/` を用いてメインテキストの文字毎にルビを割り付ける機能を実装
* 2025年11月12日
	* 字幕が AviUtl2 の制御文字列 `<s>` を含む場合に正しい字幅を取得できない場合があった問題を修正
